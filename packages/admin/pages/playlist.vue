  ...
  if (viewPlaylistData.value.find(song => song.$id === changes.$id) && Object.keys(changes).length > 1) {
    if (changesStaged) {
      ...
      for (const field in changes) {
        // @ts-ignore
        if (field !== '$id' && changes[field] !== changesStaged[field]) { diffFromStaged = true; break }
      }
      if (diffFromStaged) {
    ...
    if (changes.$id.includes('-')) { diffFromUnmodified = true } else {
      for (const field in changes) {
      // @ts-ignore
        if (field !== '$id' && changes[field] !== songUnmodified[field]) { diffFromUnmodified = true; break }
      }
